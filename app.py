...
def esegui_calcolo_deterministico(dati_geo):
    ...
    luogo_str, qsk, zona_vento, pressione_vento_str, zona_sismica, altitudine_stimata = estrai_parametri_ntc_da_coordinate_e_comune(lat, lon, comune)
    pressione_vento = float(pressione_vento_str.split()[0])
    ...
    # ---------------------------------------------------------
    # 3. DIMENSIONAMENTO ARCARECCI (In Luce vs Sopra i telai)
    # ---------------------------------------------------------
    pos_arcarecci = dati_geo.get('posizione_arcarecci', 'Sopra i telai (Continuo)')
    ...
    if "In luce" in pos_arcarecci:
        M_ed_arc = (q_arc * interasse**2) / 8.0
    else:
        M_ed_arc = (q_arc * interasse**2) / 10.0

    W_req_arc = (M_ed_arc * 100) / 27.5 # cm3 (Acciaio S275)
    ...
    # ---------------------------------------------------------
    q_ed = interasse * q_tot_copertura_mq
    ...
    # ---------------------------------------------------------
    # 5. OTTIMIZZAZIONE SNELLA PILASTRI (Stress & Deformata H/150)
    # ---------------------------------------------------------
    ...
    b_pil_perim_cm = 20
    h_pil_perim_cm = 28 # Partenza da sezione snella
    while True:
    ...
    h_pil_interm_cm = max(28, ((int(h_legno_cm * 0.50) + 3) // 4) * 4)
    b_pil_interm_cm = 20
    ...
    # --- DIMENSIONAMENTO MONTANTI TIMPANO ---
    ...
    # --- DIMENSIONAMENTO MONTANTI PARETI LONGITUDINALI ---
    ...
    if h_colmo <= 6.5:
        montante_legno = "Sezione 14x14 cm (GL24h)"
        montante_acciaio = "HEA 120 o Tubolare 120x120x4"
        montante_cap = "Pilastrino prefabbricato 20x20 cm"
    elif h_colmo <= 9.5:
        montante_legno = "Sezione 16x16 cm (GL24h)"
        montante_acciaio = "HEA 140 o Tubolare 150x150x5"
        montante_cap = "Pilastrino prefabbricato 25x25 cm"
    elif h_colmo <= 12.5:
        montante_legno = "Sezione 16x24 cm (GL24h)"
        montante_acciaio = "HEA 180 o Tubolare 200x200x5"
        montante_cap = "Pilastrino prefabbricato 30x30 cm"
    else:
        montante_legno = "Sezione 20x28 cm (GL24h)"
        montante_acciaio = "HEA 220 o Tubolare 250x250x6"
        montante_cap = "Pilastrino prefabbricato 40x40 cm"
    ...
    risultati_deterministici = {
        ...
        "pressione_vento": pressione_vento_str,
        ...
        "pilastri_perimetrali_legno": f"Sezione {b_pil_perim_cm}x{h_pil_perim_cm} cm (Ottimizzato: drift in sommità < {limite_spostamento_cm:.1f} cm)",
        "pilastri_intermedi_legno": f"Sezione {b_pil_interm_cm}x{h_pil_interm_cm} cm (Lavorante a compressione centrica)",
        ...
        "sezione_arcarecci": f"{sez_arc} o falda legno 10x20 cm per passo {passo_arcarecci:.2f}m",
        "verifica_arcarecci": f"Verificato a flessione ({pos_arcarecci}) - M_ed: {M_ed_arc:.1f} kNm",
        ...
        "baraccatura_legno_lamellare": f"Correnti in legno lamellare GL24h sezione 12x16 cm",
        "baraccatura_legno_massiccio": f"Correnti in legno massiccio C24 sezione 14x16 cm",
        ...
        "note_tecniche": f"Predimensionamento NTC 2018 (Comune: {comune if comune else 'N.D.'}). qsk = {qsk} kN/mq. Pilastri ottimizzati a deformata H/150."
    }
    
    # Update back to dati_geo so it propagates
    dati_geo['interasse_arcarecci'] = passo_arcarecci
...
def genera_word_report(dati, distinta):
    ...
    doc.add_paragraph(f"Sviluppo Metri Lineari (ML) Baraccatura Parete Longitudinale (singola): {dati.get('ml_baraccatura_long_singola', 0)} ml")
    doc.add_paragraph(f"Sviluppo Metri Lineari (ML) Baraccatura Timpano Frontale (singolo): {dati.get('ml_baraccatura_timpani_singolo', 0)} ml")
    ...
    doc.add_paragraph(f"\nSezione consigliata montanti (Legno): {dati.get('montante_sezione_legno', 'N.D.')}")
    doc.add_paragraph(f"Sezione consigliata montanti (Acciaio): {dati.get('montante_sezione_acciaio', 'N.D.')}")
    doc.add_paragraph(f"Sezione consigliata montanti (C.a.p.): {dati.get('montante_sezione_cap', 'N.D.')}")
...
if st.button("Esegui Dimensionamento e Genera Modello 3D", type="primary"):
    if lunghezza_edificio_ui <= 0 or interasse_portali_ui <= 0 or luce_totale_ui <= 0 or altezza_gronda_ui <= 0 or altezza_colmo_ui <= 0:
        ...
        dati_base = {
            ...
            'posizione_arcarecci': posizione_arc,
        ...
        if modalita_deterministica:
            ...
            if not api_key:
                ...
                - Posizione Arcarecci: {posizione_arc}
...
if 'dati_ultimi' in st.session_state:
    ...
    if dati.get('num_montanti_timpano_singolo', 0) > 0 or dati.get('num_montanti_long_singola_parete', 0) > 0:
        ...
        with col_mt1:
            st.success(f"🌲 **Legno:** {dati.get('montante_sezione_legno')}")
        with col_mt2:
            st.warning(f"⚙️ **Acciaio:** {dati.get('montante_sezione_acciaio')}")
        with col_mt3:
            st.error(f"🏛️ **C.a.p.:** {dati.get('montante_sezione_cap')}")
